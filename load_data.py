...
def get_task_room(timesteps: maze.TimeStep):
  ...
  task_room = next(
    (i for i, row in enumerate(experiment.groups) if task_object in row), None
  )
  return task_room
...
def get_timestep(datum):
  ...
  timestep = nicejax.match_types(example=experiment.dummy_timestep, data=timestep)
...
def load_experiment_data(file_path: str):
  ...
  data_dicts = [
    ...
    if not "practice" in row["metadata"]["block_metadata"]["desc"]
  ]
  datum = data_dicts[0]
...
if __name__ == "__main__":
  ...
  def parse_arguments():
    parser = argparse.ArgumentParser(
      description="Load and summarize experiment data from a JSON file."
    )
    parser.add_argument(
      "file_path",
      nargs="?",
      default="data/data_user=42_exp=1_debug=1.json",
