...
def compute_experiment_lengths(files, plot: bool = False, condition_name: str = '', verbose: bool = False):
    ...
    for file in files:
        ...
        total_length = (end_time - start_time).total_seconds() / \
...
def get_task_room(timesteps: multitask_env.TimeStep, task_groups=None):
    ...
    task_room = next((i for i, row in enumerate(
        task_groups) if task_object in row), None)
    return task_room
...
def make_row(
    ...
    row = dict(
        ...
        manipulation=datum['metadata']['block_metadata'].get(
            'manipulation', None),
...
def make_episode_data(
    ...
    assert len(idxs) == len(set(
        idxs)), f"user_episode_idx is not unique. {len(idxs)} vs {len(set(idxs))}. max={max(idxs)}"
    #####################
    ...
    if os.path.exists(episode_data_filename) and not overwrite_episode_data:
        with open(episode_data_filename, 'rb') as f:
            ...
            episode_data = serialization.from_bytes([example_episode]*nepisodes, serialized_data)
    else:
        episode_data = [None] * len(gds.keys())
        for key in tqdm(gds.keys(), desc="Processing episodes"):
            red = raw_episode_data = gds[key]
            actions = jnp.asarray([datum['data']['action_idx']
                                  for datum in red])
            timesteps = [get_timestep(datum, example_timestep)
                         for datum in red]
            ...
            if not correct:
                ...
                if jnp.all(sorted_steps == expected_step_num):
                    ...
                    timesteps = jtu.tree_map(
                        lambda x: x[sort_indices] if isinstance(
    ...
    if os.path.exists(episode_info_filename) and not overwrite_episode_info:
        ...
        for key in gds.keys():
            ...
            reaction_times = [compute_reaction_time(
                datum) for datum in raw_episode_data]
