...
def get_algorithm_data(
  ...
  _, _, _, label2name = housemaze_experiments.exp2(
...
def get_usfa_data(
  ...
  eval_task_runner_sf = sf_task_runner.TaskRunner(
      ...
      vis_coeff=vis_coeff,
      radius=5)
  train_tasks = jnp.array([eval_task_runner_sf.task_vector(o)
...
def collect_search_episodes(
  ...
  for actions in all_actions:
      ...
      padded_actions.append(np.concatenate(
          (actions, np.array(padding, dtype=np.int32))))
