...
from analysis.housemaze_utils import (
  success,
  success_or_not_terminate,
  terminated,
  went_to_junction,
)
...
def bar_plot_results(
  ...
  errors = (
    [
      np.std(arr) / np.sqrt(len(arr)) if not model.startswith("human") else 0
...
def add_reuse_columns(df: DataFrame, overlap_threshold=0.15) -> DataFrame:
  ...
  new_df = df.with_columns([pl.Series("reuse", reuse_values)])
...
def coplot_dists(df1, df2, val, settings: dict):
  ...
  sns.histplot(
    data=mdf1[val], bins=bins, color="skyblue", alpha=0.5, ax=ax, label="User DF"
  )

  sns.histplot(
    data=mdf2[val], bins=bins, color="red", alpha=0.5, ax=ax, label="Filtered DF"
  )
...
def filter_outliers(
  ...
  if n_filtered > 0:
    users_removed = set()
    print(
      f"Filtered {n_filtered} outliers from {len(eval_mask)} rows using {method} method"
    )
...
def filter_rt_outliers(
  episodes: List[EpisodeData], rt_fn: Callable, n_std: float = 3.0
) -> List[EpisodeData]:
...
def plot_train_reaction_times_dual(
  user_df: DataFrame, stages: Optional[List[str]] = None, **kwargs
):
  title_size = kwargs.pop("title_size", DEFAULT_TITLE_SIZE)
  label_size = kwargs.pop("label_size", DEFAULT_LABEL_SIZE)
  legend_size = kwargs.pop("legend_size", DEFAULT_LEGEND_SIZE)
...
def plot_train_success_rate_histograms(
  user_df: DataFrame, stages: Optional[List[str]] = None, n_cols: int = 2, **kwargs
):
  title_size = kwargs.pop("title_size", DEFAULT_TITLE_SIZE)
  label_size = kwargs.pop("label_size", DEFAULT_LABEL_SIZE)
  legend_size = kwargs.pop("legend_size", DEFAULT_LEGEND_SIZE)
  ...
  for i, name in enumerate(stages):
    ...
    if success_rates:
      n, _, _ = axs[row, col].hist(success_rates, bins=20, edgecolor="black")
...
def plot_episode_counts(
  ...
  def plot_histogram(ax, data, title):
    n, bins, patches = ax.hist(
      data,
      bins=range(min(data), max(data) + 2, 1),
      edgecolor="black",
      align="left",
    )
    ax.set_title(title, fontsize=title_size)
...
def plot_episode_length_seconds(user_df: DataFrame, settings=None, **kwargs):
  ...
  overall_episode_lengths = [
    length
    for s in stages
    for user_lengths in user_episode_lengths[s].values()
  ...
  def plot_histogram(ax, data, title):
    ...
    ax.axvline(
      ...
      linestyle="dashed",
      linewidth=3,
      label=f"Median: {median_length:.2f}s",
    )
    ax.legend(fontsize=legend_size)
  ...
  for i, stage in enumerate(stages):
    data = [
      length
      for user_lengths in user_episode_lengths[stage].values()
      for length in user_lengths
    ]
  ...
  _, _, freq = plot_histogram(axs[-1, -1], overall_episode_lengths, "Overall")
  max_frequency = max(max_frequency, freq)
...
def reaction_times_difference(
  ...
  for user in users:
    ...
    if len(cond1_user.episodes) != len(cond2_user.episodes):
      print(
        f"Skipping {user}: {len(cond1_user.episodes)} != {len(cond2_user.episodes)}"
      )
      continue
  ...
  for rt_type in rt_types:
    user_means[f"{rt_type}_diff"] = (
      user_means[f"{rt_type}_rt_cond2"] - user_means[f"{rt_type}_rt_cond1"]
    )
  ...
  def plot_rt_difference(ax, data, rt_type, i=0):
    ...
    ax.axhline(
      y=mean, color="blue", linestyle="-", linewidth=2, label=f"Mean: {mean:.2f}"
    )

    # Add standard deviation range
    ax.axhspan(mean - std, mean + std, alpha=0.2, color="blue", label=f"SE: {std:.2f}")
...
def create_success_termination_results_m3(
  user_df: DataFrame, model_df: DataFrame, **kwargs
):
  ...
  def model_fn(e):
    return jax.vmap(success_fn)(e)

  # Success rate data
  data = {
    "human": 100 * succeeded,
    #'human_terminate': finished,
    "qlearning": model_df.apply(
      fn=model_fn, output_transform=post_fn, algo="qlearning", **model_setting
    ),
    "usfa": model_df.apply(
      fn=model_fn, output_transform=post_fn, algo="usfa", **model_setting
    ),
    "dyna": model_df.apply(
      fn=model_fn, output_transform=post_fn, algo="dynaq_shared", **model_setting
    ),
    "bfs": model_df.apply(
      fn=model_fn, output_transform=post_fn, algo="bfs", **model_setting
    ),
    "dfs": model_df.apply(
      fn=model_fn, output_transform=post_fn, algo="dfs", **model_setting
    ),
  }
...
def _reuse_bar_plot_results(
  ...
  # Plot bars
  ax.bar(
    [model_names.get(model, model) for model in data.keys()],
  ...
  ax.scatter(
    [x_pos + j for j in x_jitter], human_means, color="black", alpha=0.5, zorder=3
  )
  ...
  stats_text = (
    f"Human mean: {human_mean:.3f} ± {human_se:.3f}\n"
    f"t-stat: {t_stat:.3f}\n"
...
def create_bar_plot_results_m1(
  user_df: DataFrame, model_df: DataFrame, ax=None, ylim=None
):
...
def create_bar_plot_results_m3(
  user_df: DataFrame, model_df: DataFrame, ax=None, ylim=None
):
...
def create_success_termination_results_m1(
  user_df: DataFrame, model_df: DataFrame, **kwargs
):
  manipulation = 1
...
def plot_m2_example(user_df: DataFrame):
  ...
  # SHOULD BE SMALLER
  cond1 = subset.filter(
    maze="big_m2_maze2_onpath_(F,F)", manipulation=2, eval=True, condition=1
  )  # On-path
  # SHOULD BE LARGER
  cond2 = subset.filter(
    maze="big_m2_maze2_offpath_(F,F)", manipulation=2, eval=True, condition=2
  )  # Off-path
...
def m2_reaction_time_difference(
  user_df: DataFrame, rt_types=["first", "avg"], **kwargs
):
  subset = user_df.filter_by_group(
    input_episode_filter=group_filter_fn,
    output_episode_filter=lambda e: not success_or_not_terminate(e),
...
def plot_m4_example(
  user_df: DataFrame, setting: str = "short", version: str = "regular"
):
...
def m4_reaction_times(user_df: DataFrame, setting="short", rt_type="speed", ylim=None):
  ...
  def create_plots(output_filter_fn, title_suffix):
    ...
    fig.suptitle(
      f"{setting.capitalize()} Maze - {title_suffix} - {rt_type.capitalize()} Reaction Time",
    ...
    def plot_rt_by_action(ax, episodes, title):
      ...
      box_colors = [
        model_colors.get(key2model.get(label.lower(), ""), "#333333")
        for label in labels
      ]
      ...
      ax.set_ylabel(
        f"{rt_type.capitalize()} Reaction Time (s)", fontsize=DEFAULT_LABEL_SIZE
      )
      ax.tick_params(axis="both", which="major", labelsize=DEFAULT_LABEL_SIZE)
...
def m4_action_reaction_times(user_df: DataFrame, setting="short", rt_type="speed"):
  assert setting in ["short", "long"]
  assert rt_type in ["speed", "first"]
  ...
  sns.boxplot(data=box_data, ax=ax, width=0.5, palette=["#E69F00", "#E69F00"])
  sns.stripplot(data=box_data, ax=ax, color="black", alpha=0.5, jitter=True)

  # Customize the plot
  ax.set_title(f"{setting.capitalize()}", fontsize=DEFAULT_TITLE_SIZE)
...
def m4_reaction_time_difference(
  ...
  **kwargs,
  ...
  return reaction_times_difference(same_cond, diff_cond, rt_types=rt_types, **kwargs)


def m4_action_reaction_time_difference(
  user_df: DataFrame, setting="short", rt_types=["first", "avg"], **kwargs
):
  assert setting in ["short", "long"]
...
def plot_m3_episode_length_histogram(user_df: DataFrame, **kwargs):
  ...
  # Customize the plot
  ax.set_title("Distribution of Episode Lengths (Manipulation 3)", fontsize=title_size)
  ax.set_xlabel("Episode Length (seconds)", fontsize=label_size)
  ax.set_ylabel("Frequency", fontsize=label_size)
  ax.tick_params(axis="both", which="major", labelsize=label_size)
  ax.grid(True, alpha=0.3)
  ...
  **kwargs,
  ...
  def get_conditions(setting):
    ...
    if filter_by_action:
      ...
      def same_episode_filter(e):
        return e.actions[0] != same_cond_action

      def diff_episode_filter(e):
        return e.actions[0] != diff_cond_action
    ...
    same_cond = subset.filter(
      name=f"big_m4_maze_{setting}_eval_same", episode_filter=same_episode_filter
    )
    diff_cond = subset.filter(
      name=f"big_m4_maze_{setting}_eval_diff", episode_filter=diff_episode_filter
    )
  ...
  users = (
    set(short_same_cond["user_id"].unique())
  ...
  for user in users:
    ...
    if (
      len(short_same_user.episodes) > 0
      and len(short_diff_user.episodes) > 0
      and len(long_same_user.episodes) > 0
      and len(long_diff_user.episodes) > 0
    ):
      for rt_type in rt_types:
        ...
        if all(
          good_number(rt)
          for rt in [rt_short_same, rt_short_diff, rt_long_same, rt_long_diff]
        ):
          rt_data[rt_type]["short"].append(rt_short_diff - rt_short_same)
          rt_data[rt_type]["long"].append(rt_long_diff - rt_long_same)
        else:
          print(
            f"Skipping {user}, short same: {rt_short_same:.2f}, short diff: {rt_short_diff:.2f}, long same: {rt_long_same:.2f}, long diff: {rt_long_diff:.2f}"
          )
  ...
  for i, (ax, rt_type) in enumerate(zip(axs, rt_types)):
    plot_rt_comparison(
      ax, rt_data[rt_type], f"{rt_type.capitalize()} Reaction Time Difference", i
    )
