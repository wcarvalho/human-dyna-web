...
for config in craftax_experiment_configs.PATHS_CONFIGS:
    ...
    if os.path.exists(cache_file):
      ...
      env_params = craftax_experiment_configs.make_block_env_params(
        config, experiment.default_params).replace(
...
def get_path_reuse_df(
  ...
  sub_df = user_df.filter_by_group(
      ...
      output_settings=dict(manipulation='paths', tell_reuse=tell_reuse, eval_map=int(eval_map)),
      group_key="user_id",
    ).filter(eval=True)
  sub_df = experiment_analysis.fix_reuse_column(sub_df)
  sub_df = sub_df.filter(pl.col("success").is_not_null() & pl.col("reuse").is_not_null())
  return sub_df
...
def visualize_user_path_reuse(df: DataFrame, user_id: int, idx=None):
  ...
  for i in range(len(test_mazes)):
    ...
    train_df = user_df.filter(name=train_maze, room=0, eval=False, success=1, start_pos=start_pos)
