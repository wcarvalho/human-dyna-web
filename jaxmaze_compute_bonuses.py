...
def main(files, outfile, debug=False):
  ...
  for manipulation in range(0, 4):
    ...
    eval_df = eval_df.with_columns(pl.col("total_rt").lt(20).cast(pl.Int64).alias("bonus"))
  ...
  def maze_names(setting, blind):
    condition1 = f"big_m4_maze_{setting}_eval_same_blind" if blind else f"big_m4_maze_{setting}_eval_same"
    condition2 = f"big_m4_maze_{setting}_eval_diff_blind" if blind else f"big_m4_maze_{setting}_eval_diff"
      ...
      for suffix in ['F,F', 'T,F', 'F,T', 'T,T']:
        ...
        for condition in [condition1, condition2]:
          ...
          if maze_df.height > 0:
              ...
              maze_df = maze_df.with_columns(pl.col("total_rt").lt(threshold).cast(pl.Int64).alias("bonus"))
  ...
  bonus_df = pl.DataFrame({
      "worker_id": list(bonus_values.keys()),
      "total_bonus": list(bonus_values.values())
  })
