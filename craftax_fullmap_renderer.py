...
def render_craftax_pixels(state, block_pixel_size, do_night_noise=True):
    ...
    map_pixels_item_indexes = jnp.repeat(
        map_pixels_item_indexes, repeats=3, axis=2)
    ...
    def _add_item_type_to_pixels(pixels, item_index):
        ...
        pixels = pixels * \
            (1 - full_map_texture[:, :, 3] * mask[:, :, 0])[:, :, None]
        pixels = (
            pixels
            + full_map_texture[:, :, :3] * mask *
            full_map_texture[:, :, 3][:, :, None]
        )
    ...
    def _add_projectile_to_pixels(carry, projectile_index):
        ...
        flipped_projectile_texture_alpha = jnp.flip(
            projectile_texture_alpha, axis=0)
        ...
        transposed_projectile_texture = jnp.transpose(
            projectile_texture, (1, 0, 2))
        transposed_projectile_texture_alpha = jnp.transpose(
            projectile_texture_alpha, (1, 0, 2)
        )

        projectile_texture = jax.lax.select(
            projectile_directions[state.player_level,
    ...
    if do_night_noise:
        night_noise = (
            jax.random.uniform(
                state.state_rng, night_pixels.shape[:2]) * 95 + 32
        )
        ...
        night_mask = textures["night_noise_intensity_texture"] * \
            night_intensity
    ...
    # RENDER INVENTORY
    inv_pixel_left_space = (
        block_pixel_size - int(0.8 * block_pixel_size)) // 2
    ...
    inv_pixels = _render_two_digit_number(
        inv_pixels, state.player_energy, 3, 0)
    ...
    inv_pixels = _render_two_digit_number(
        inv_pixels, state.inventory.wood, 0, 2)
    ...
    inv_pixels = _render_two_digit_number(
        inv_pixels, state.inventory.stone, 1, 2)
    ...
    inv_pixels = _render_two_digit_number(
        inv_pixels, state.inventory.coal, 0, 1)
    ...
    inv_pixels = _render_two_digit_number(
        inv_pixels, state.inventory.iron, 1, 1)
    ...
    inv_pixels = _render_two_digit_number(
        inv_pixels, state.inventory.diamond, 2, 1)
    ...
    inv_pixels = _render_two_digit_number(
        inv_pixels, state.inventory.sapphire, 3, 1)
    ...
    inv_pixels = _render_two_digit_number(
        inv_pixels, state.inventory.ruby, 4, 1)
    ...
    inv_pixels = _render_two_digit_number(
        inv_pixels, state.inventory.sapling, 5, 1)
    ...
    inv_pixels = _render_two_digit_number(
        inv_pixels, state.inventory.arrows, 6, 2)
    ...
    inv_pixels = _render_two_digit_number(
        inv_pixels, state.inventory.torches, 2, 2)
    ...
    inv_pixels = _render_two_digit_number(
        inv_pixels, state.inventory.books, 3, 2)
    ...
    arrow_enchantment_level = state.bow_enchantment * \
        (state.inventory.arrows > 0)
...
def render_craftax_text(state: EnvState):
    ...
    def _add_mob_to_map(carry, mob_index):
        ...
        on_screen = jnp.logical_and(
            local_position >= 0, local_position < jnp.array(
                [OBS_DIM[0], OBS_DIM[1]])
        ).all()
        on_screen *= mobs.mask[mob_index]

        mob_identifier = mob_class_index * \
            mob_types_per_class + mobs.type_id[mob_index]
    ...
    (mob_map, _, _), _ = jax.lax.scan(
        ...
        (
            ...
            jax.tree_util.tree_map(
                lambda x: x[state.player_level], state.melee_mobs),
            0,
        ),
        jnp.arange(state.melee_mobs.mask.shape[1]),
    )
    (mob_map, _, _), _ = jax.lax.scan(
        ...
        (
            mob_map,
            jax.tree_util.tree_map(
                lambda x: x[state.player_level], state.passive_mobs),
            1,
        ),
        jnp.arange(state.passive_mobs.mask.shape[1]),
    )
    (mob_map, _, _), _ = jax.lax.scan(
        ...
        (
            mob_map,
            jax.tree_util.tree_map(
                lambda x: x[state.player_level], state.ranged_mobs),
            2,
        ),
        jnp.arange(state.ranged_mobs.mask.shape[1]),
    )
    (mob_map, _, _), _ = jax.lax.scan(
        ...
        (
            mob_map,
            jax.tree_util.tree_map(
                lambda x: x[state.player_level], state.mob_projectiles
            ),
            3,
        ),
        jnp.arange(state.mob_projectiles.mask.shape[1]),
    ...
    (mob_map, _, _), _ = jax.lax.scan(
        ...
        (
            mob_map,
            jax.tree_util.tree_map(
                lambda x: x[state.player_level], state.player_projectiles
            ),
            4,
        ),
        jnp.arange(state.player_projectiles.mask.shape[1]),
    ...
    light_map_view = jax.lax.dynamic_slice(
        padded_light_map, tl_corner, OBS_DIM) > 0.05
        ...
        for y in range(OBS_DIM[1]):
            ...
            if light_map_view[x, y]:
                ...
                if item_map_view[x, y] != ItemType.NONE.value:
                    text_obs += ItemType(item_map_view[x, y]
                                         ).name.lower() + " on "
    ...
    if state.inventory.sword > 0:
        ...
        text_obs += (
            " with " +
            level_to_enchantment(state.sword_enchantment) + " enchantment\n"
        )
    if state.inventory.bow > 0:
        text_obs += (
            "Bow with " +
            level_to_enchantment(state.bow_enchantment) + " enchantment\n"
        )
