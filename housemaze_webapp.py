...
def stage_name(stage):
   ...
   return f'block {block_idx}. {block_name}: {stage.name}'
...
async def save_data(final_save=True, feedback=None, **kwargs):
    ...
    if final_save:
      user_storage = nicewebrl.nicejax.make_serializable(
...
def initalize_user(user_info):
  ...
  app.storage.user['block_progress'] = app.storage.user.get(
      'block_progress', 0.)
  ...
  if not stage_order:
    ...
    # example block order
    # [e.g., 0, 1, 3, 2]
    block_order, stage_order = experiment.generate_block_stage_order(
        init_rng_key)
    block_order_to_idx = {str(i): int(idx)
                          for idx, i in enumerate(block_order)}
  ...
  for block, block_pieces in stage_names.items():
     for gloabl_idx, (idx_in_block, name) in block_pieces.items():
        stage_to_block_idx[gloabl_idx] = (
            idx_in_block % len(block_pieces), name)
