...
class SuccessTrackingAutoResetWrapper(utils.AutoResetWrapper):
    ...
    def reset(self, key, params):
        ...
        mask = task_object_mask(
            objects_to_mask, self.task_runner.task_objects)
        probs = mask/(mask.sum())
        ...
        return update_timestep(
            ...
            successes=jnp.zeros_like(
                init_timestep.state.task_state.features))
    ...
    def __auto_reset(self, key, params, timestep):
        ...
        successes, remaining, mask = self.compute_successes_remaining(
            timestep, params)
