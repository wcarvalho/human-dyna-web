...
TEXTURE_CACHE_FILE = os.path.join(
  original_constant_directory, "assets", "fullmap_texture_cache.pbz2"
)
...
def load_texture(filename, block_pixel_size):
  filename = os.path.join(original_constant_directory, "assets", filename)
...
def load_all_textures(block_pixel_size):
  ...
  block_textures = jnp.array(
    [load_texture(fname, block_pixel_size)[:, :, :3] for fname in block_texture_names]
  )
