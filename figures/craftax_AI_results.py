...
from configs import default_colors, model_colors, DIRECTORY

DIRECTORY = os.path.join(
  DIRECTORY, 'craftax_AI_results', 'data')
    ...
    for model, group in tqdm(model_to_group.items(), desc="Models", leave=True):
        ...
        if debug:
            cache_file = os.path.join(
                DIRECTORY, f'{model}_{group}_debug_raw.json')
        else:
            cache_file = os.path.join(
                DIRECTORY, f'{model}_{group}_raw.json')
...
def plot_achievement_bars(df, n=64, fig=None, ax=None, figsize=(12, 5), show_legend=True):
    ...
    ax.set_xlim(-.75, len(x_pos) - 0.5)

    ax.grid(True, alpha=0.3)
    if show_legend:
        ax.legend(loc='upper right', ncol=1, fontsize=10)
    ax.set_title(f'Per-Achievement Generalization Success Rates given {n} Unique Training Environments', 
                fontsize=14, 
                pad=5)
...
def plot_training_envs_score(df, ntraining_envs, ax=None, figsize=(5, 5), show_legend=True, evaluation=True, ylim=None):
    # Create figure and axis if not provided
    if ax is None:
        fig, ax = plt.subplots(figsize=figsize)
    else:
