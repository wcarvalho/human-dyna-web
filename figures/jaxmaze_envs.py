...
def render_path(
  maze_str, goal, ax=None, rng=None, plot_image=True, plot_path=True, include_spawn=True, use_title=False, arrow_color='g', star_color='', star_at_start=False, **kwargs
):
  level_init = utils.from_str(maze_str, char2key)
  image = renderer.create_image_from_grid(
    *level_init,
  ...
  if path is None:
    ...
    if plot_path:
      renderer.place_arrows_on_image(
        image, path, actions, *level_init[0].shape[:2], ax=ax, arrow_color=arrow_color, plot_image=plot_image
      )
    if star_at_start:
      ...
      ax.plot(center_x, center_y, marker='*', color=star_color, 
              markersize=scale_x*(.5),  # Scale star size relative to cell size
              markeredgecolor=star_color, markeredgewidth=scale_x/20)
  ax.axis("off")
  ax.set_xticks([])
  ax.set_yticks([])
...
if __name__ == "__main__":
  ...
  def plot_two_paths(ax, reversal):
    render_path(utils.reverse(mazes.big_m3_maze1, *reversal), goal=task_objects[0], ax=ax, arrow_color=TRAIN_COLOR, star_at_start=True, star_color='white')
    render_path(utils.reverse(mazes.big_m3_maze1, *reversal), goal=task_objects[1], ax=ax, arrow_color=EVAL_COLOR, star_at_start=True, star_color='white')
  ...
  def plot_juncture_near_known(ax, reversal):
    render_path(utils.reverse(mazes.big_m4_maze_short, *reversal), goal=task_objects[0], ax=ax, arrow_color=TRAIN_COLOR, star_at_start=True)
    render_path(utils.reverse(mazes.big_m4_maze_short_eval_same, *reversal), goal=task_objects[1], ax=ax, plot_image=False, arrow_color=EVAL_COLOR, star_at_start=True)
    render_path(utils.reverse(mazes.big_m4_maze_short_eval_diff, *reversal), goal=task_objects[2], ax=ax, plot_image=False, arrow_color=EVAL2_COLOR, star_at_start=True)

  plot_single_and_rotations("2.juncture_manipulation_near_known", plot_juncture_near_known, save_figure)

  def plot_juncture_near_unknown(ax, reversal):
    render_path(utils.reverse(mazes.big_m4_maze_short_blind, *reversal), goal=task_objects[0], ax=ax, arrow_color=TRAIN_COLOR, star_at_start=True)
    render_path(utils.reverse(mazes.big_m4_maze_short_eval_same_blind, *reversal), goal=task_objects[1], ax=ax, plot_image=False, arrow_color=EVAL_COLOR, star_at_start=True)
    render_path(utils.reverse(mazes.big_m4_maze_short_eval_diff_blind, *reversal), goal=task_objects[2], ax=ax, plot_image=False, arrow_color=EVAL2_COLOR, star_at_start=True)

  plot_single_and_rotations("2.juncture_manipulation_near_unknown", plot_juncture_near_unknown, save_figure)

  def plot_juncture_far_known(ax, reversal):
    render_path(utils.reverse(mazes.big_m4_maze_long, *reversal), goal=task_objects[0], ax=ax, arrow_color=TRAIN_COLOR, star_at_start=True)
    render_path(utils.reverse(mazes.big_m4_maze_long_eval_same, *reversal), goal=task_objects[1], ax=ax, plot_image=False, arrow_color=EVAL_COLOR, star_at_start=True)
  ...
  def plot_start_manipulation(ax, reversal):
    render_path(utils.reverse(mazes.big_m2_maze2, *reversal), goal=task_objects[0], ax=ax, arrow_color=TRAIN_COLOR, star_at_start=True)
    render_path(utils.reverse(mazes.big_m2_maze2_onpath, *reversal), goal=task_objects[0], ax=ax, plot_image=False, plot_path=False, arrow_color=EVAL_COLOR, star_at_start=True)
    render_path(utils.reverse(mazes.big_m2_maze2_offpath, *reversal), goal=task_objects[0], ax=ax, plot_image=False, plot_path=False, arrow_color=EVAL2_COLOR, star_at_start=True)

  plot_single_and_rotations("3.start_manipulation", plot_start_manipulation, save_figure)

  ########################################################
  # Shortcut Manipulation
  ########################################################
  def plot_shortcut_manipulation(ax, reversal):
    render_path(utils.reverse(mazes.big_m1_maze3_shortcut, *reversal), goal=task_objects[1], ax=ax, arrow_color=EVAL_COLOR, star_at_start=True, star_color='white')
    render_path(utils.reverse(mazes.big_m1_maze3, *reversal), goal=task_objects[0], ax=ax, arrow_color=TRAIN_COLOR, star_at_start=True, star_color='white')

  plot_single_and_rotations("4.shortcut_manipulation", plot_shortcut_manipulation, save_figure)
