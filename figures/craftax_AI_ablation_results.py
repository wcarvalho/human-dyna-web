...
def get_run_history(model_to_group, key='evaluator_performance-512/0.score', window_size=5, debug=False):
    """
    ...
    for model, group in tqdm(model_to_group.items(), desc="Models"):
        ...
        for run_id in model_df['run_id'].unique():
            ...
            if len(run_slice) >= window_size:
                ...
                run_slice[numeric_cols] = run_slice[numeric_cols].rolling(window=window_size, min_periods=1).mean()
...
def plot_performance_curves(data, ax, key='evaluator_performance-512/0.score', 
                           xlabel='Actor Steps', ylabel='Score', title=None, model_to_line=None):
    """
    ...
    if max_step > 1e6:
        ax.xaxis.set_major_formatter(lambda x, pos: f"{x/1e6:.0f}M" if x >= 1e6 else f"{x/1e3:.0f}K")
    elif max_step > 1e3:
